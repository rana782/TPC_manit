...
const demoCompanyProfiles_1 = require("../utils/demoCompanyProfiles");
const router = (0, express_1.Router)();
const prisma = new client_1.PrismaClient();
router.get('/seed-ui', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const student = yield prisma.student.findUnique({ where: { userId: user.id } });
        if (student) {
            yield prisma.student.update({ where: { id: student.id }, data: { cgpa: 8.5, linkedin: 'https://linkedin.com/in/uitester', isLocked: false, lockedReason: null } });
        }
        // Ensure resumes exist
        const resumes = yield prisma.resume.findMany({ where: { studentId: student.id } });
        if (resumes.length === 0) {
            yield prisma.resume.create({ data: { studentId: student.id, fileName: 'Frontend.pdf', fileUrl: '/uploads/fe.pdf', isActive: true, roleName: 'Frontend Engineer Profile. Strong React skills.' } });
            yield prisma.resume.create({ data: { studentId: student.id, fileName: 'Backend.pdf', fileUrl: '/uploads/be.pdf', isActive: true, roleName: 'Backend Engineer Profile.' } });
            yield prisma.resume.create({ data: { studentId: student.id, fileName: 'Random.pdf', fileUrl: '/uploads/r.pdf', isActive: true, roleName: 'I like painting.' } });
        }
        // Ensure SPOC and job exist
        let spoc = yield prisma.user.findUnique({ where: { email: 'ui_spoc@example.com' } });
        if (!spoc) {
            spoc = yield prisma.user.create({
                data: { email: 'ui_spoc@example.com', password: yield bcrypt_1.default.hash(PASS, 10), role: 'SPOC', isVerified: true, permJobCreate: true, permExportCsv: true, permLockProfile: true },
            });
        }
        else {
            // Ensure existing SPOC has permissions
            yield prisma.user.update({ where: { id: spoc.id }, data: { permJobCreate: true, permExportCsv: true, permLockProfile: true } });
        }
        ...
        const jobs = yield prisma.job.findMany({ where: { postedById: spoc.id } });
        if (jobs.length === 0) {
            ...
            if (innovate) {
                yield prisma.job.update({ where: { id: innovate.id }, data: { customQuestions: customQ, requiredProfileFields: reqFields, eligibleBranches: eligBranches } });
            }
        }
        // Clean existing applications by this student for InnovateTech so apply tests start fresh
        if (student) {
            const innovateJob = yield prisma.job.findFirst({ where: { companyName: 'InnovateTech' } });
            if (innovateJob) {
                yield prisma.jobApplication.deleteMany({ where: { studentId: student.id, jobId: innovateJob.id } });
            }
        }
...
router.get('/seed-ui-locked', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!user) {
            ...
            yield prisma.student.update({ where: { userId: user.id }, data: { isLocked: true, lockedReason: 'Placed at Google via on-campus drive' } });
        }
...
router.get('/seed-ui-timeline', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        const PASS = 'Password@123';
        // 1. Ensure coordinator exists
        let coord = yield prisma.user.findUnique({ where: { email: 'ui_coord@example.com' } });
        if (!coord) {
            coord = yield prisma.user.create({
                data: { email: 'ui_coord@example.com', password: yield bcrypt_1.default.hash(PASS, 10), role: 'COORDINATOR', isVerified: true },
            });
        }
        // 2. Get student + InnovateTech job
        const studentUser = yield prisma.user.findUnique({ where: { email: 'ui_student@example.com' } });
        if (!studentUser)
            return res.status(400).json({ success: false, message: 'Run /seed-ui first' });
        const student = yield prisma.student.findUnique({ where: { userId: studentUser.id } });
        if (!student)
            return res.status(400).json({ success: false, message: 'Student profile missing' });
        // Unlock student for this test
        yield prisma.student.update({ where: { id: student.id }, data: { isLocked: false, lockedReason: null } });
        const innovateJob = yield prisma.job.findFirst({ where: { companyName: 'InnovateTech' } });
        if (!innovateJob)
            return res.status(400).json({ success: false, message: 'InnovateTech job missing — run /seed-ui first' });
        // Ensure job has ON_CAMPUS placement mode so declareResults locks students
        yield prisma.job.update({ where: { id: innovateJob.id }, data: { placementMode: 'ON_CAMPUS' } });
        // 3. Clean old stages for this job
        yield prisma.jobStage.deleteMany({ where: { jobId: innovateJob.id } });
        // 4. Clean old placement records, profile locks for this student
        yield prisma.placementRecord.deleteMany({ where: { studentId: student.id } }).catch(() => { });
        yield prisma.profileLock.deleteMany({ where: { studentId: student.id } }).catch(() => { });
        // 5. Ensure student has an application for InnovateTech
        const resumes = yield prisma.resume.findMany({ where: { studentId: student.id } });
        let app = yield prisma.jobApplication.findFirst({ where: { studentId: student.id, jobId: innovateJob.id } });
        if (!app) {
            app = yield prisma.jobApplication.create({
                data: {
                    studentId: student.id,
                    jobId: innovateJob.id,
                    resumeId: ((_a = resumes[0]) === null || _a === void 0 ? void 0 : _a.id) || '',
            ...
            yield prisma.jobApplication.update({ where: { id: app.id }, data: { status: 'APPLIED', currentStageIndex: 0 } });
        }
...
router.get('/seed-ui-locking', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        const PASS = 'Password@123';
        // Ensure coordinator
        ...
        yield prisma.student.update({ where: { id: student.id }, data: { isLocked: false, lockedReason: null, placementType: null } });
        yield prisma.profileLock.deleteMany({ where: { studentId: student.id } }).catch(() => { });
        yield prisma.placementRecord.deleteMany({ where: { studentId: student.id } }).catch(() => { });
        ...
        if (!app) {
            app = yield prisma.jobApplication.create({
                data: {
                    studentId: student.id, jobId: innovateJob.id, resumeId: ((_a = resumes[0]) === null || _a === void 0 ? void 0 : _a.id) || '',
...
router.get('/seed-ui-coordinator', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const PASS = 'Password@123';
        ...
        if (!coord) {
            ...
            yield prisma.user.update({
                where: { id: coord.id },
                data: {
                    password: yield bcrypt_1.default.hash(PASS, 10),
                    role: 'COORDINATOR',
                    isVerified: true
                }
            });
        ...
        let pendingSpoc = yield prisma.user.findUnique({ where: { email: PENDING_EMAIL } });
        if (!pendingSpoc) {
            pendingSpoc = yield prisma.user.create({
                data: { email: PENDING_EMAIL, password: yield bcrypt_1.default.hash(PASS, 10), role: 'SPOC', isVerified: false, permJobCreate: false, permExportCsv: false, permLockProfile: false },
            });
        }
        else {
            // Reset to unverified for re-testing
            yield prisma.user.update({ where: { id: pendingSpoc.id }, data: { isVerified: false, permJobCreate: false, permExportCsv: false, permLockProfile: false } });
        }
        ...
        if (studentUser) {
            const student = yield prisma.student.findUnique({ where: { userId: studentUser.id } });
            if (student) {
                yield prisma.student.update({ where: { id: student.id }, data: { isLocked: true, lockedReason: 'Locked for override test', placementType: 'ON_CAMPUS' } });
                // Ensure a ProfileLock record exists for the override to find
                const existingLock = yield prisma.profileLock.findFirst({ where: { studentId: student.id, isActive: true } });
                if (!existingLock) {
                    const spoc = yield prisma.user.findUnique({ where: { email: 'ui_spoc@example.com' } });
                    if (spoc) {
                        yield prisma.profileLock.create({
...
router.get('/seed-ui-notifications', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        const PASS = 'Password@123';
        const fs = require('fs');
        ...
        const student = yield prisma.student.findUnique({ where: { userId: studentUser.id } });
        if (!student)
            return res.status(400).json({ success: false, message: 'Student missing' });
        // Unlock + set phone for WhatsApp
        yield prisma.student.update({ where: { id: student.id }, data: { isLocked: false, lockedReason: null, phone: '9876543210' } });
        // 3. Clear old notifications and logs for this student
        yield prisma.notification.deleteMany({ where: { userId: studentUser.id } });
        yield prisma.notificationLog.deleteMany({ where: { userId: studentUser.id } });
        // 4. Enable WhatsApp integration
        yield prisma.systemSetting.upsert({ where: { key: 'WHATSAPP_ENABLED' }, create: { key: 'WHATSAPP_ENABLED', value: 'true' }, update: { value: 'true' } });
        ...
        const resumes = yield prisma.resume.findMany({ where: { studentId: student.id } });
        yield prisma.jobApplication.deleteMany({ where: { studentId: student.id, jobId: innovateJob.id } });
        const app = yield prisma.jobApplication.create({
            data: {
                studentId: student.id, jobId: innovateJob.id, resumeId: ((_a = resumes[0]) === null || _a === void 0 ? void 0 : _a.id) || '',
                applicationData: JSON.stringify({ resume: '/uploads/fe.pdf' }), extraAnswers: JSON.stringify({}),
                status: 'APPLIED', currentStageIndex: 0, atsScore: 76, atsExplanation: 'Good match', atsMatchedKeywords: JSON.stringify(['react']),
...
router.get('/seed-ui-analytics', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        // 1. Get test entities
        const studentUser = yield prisma.user.findUnique({ where: { email: 'ui_student@example.com' } });
        if (!studentUser)
        ...
        if (!innovateJob)
            return res.status(400).json({ success: false, message: 'Run /seed-ui first' });
        const spocUser = yield prisma.user.findUnique({ where: { email: 'ui_spoc@example.com' } });
        ...
        let app = yield prisma.jobApplication.findFirst({ where: { studentId: student.id, jobId: innovateJob.id } });
        if (!app) {
            const resume = yield prisma.resume.findFirst({ where: { studentId: student.id } });
            if (resume) {
                app = yield prisma.jobApplication.create({
            ...
            yield prisma.jobApplication.update({ where: { id: app.id }, data: { status: 'ACCEPTED', currentStageIndex: 0, atsScore: 72 } });
        }
        // 4. Ensure PlacementRecord exists
        yield prisma.placementRecord.deleteMany({ where: { studentId: student.id, jobId: innovateJob.id } });
        yield prisma.placementRecord.create({
            data: { studentId: student.id, jobId: innovateJob.id, companyName: innovateJob.companyName, role: innovateJob.role, ctc: '18 LPA', placementMode: 'ON_CAMPUS', createdBySpocId: (spocUser === null || spocUser === void 0 ? void 0 : spocUser.id) || '' }
        });
        // 5. Ensure Alumni record exists for InnovateTech
        yield prisma.alumni.deleteMany({ where: { studentId: student.id, companyName: 'InnovateTech' } });
        yield prisma.alumni.create({
            data: {
...
router.get('/seed-ui-linkedin', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const fs = require('fs');
        const path = require('path');
        // 1. Clear mock server log
        ...
        yield prisma.placementRecord.create({
            data: {
                ...
                createdBySpocId: (spoc === null || spoc === void 0 ? void 0 : spoc.id) || ''
            }
        });
        // 5. Clear old PlacementAnnouncementLogs
        yield prisma.placementAnnouncementLog.deleteMany({ where: { jobId: innovateJob.id } });
...
router.get('/seed-spoc-round3', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const PASS = 'Password@123';
        const stageDates = [
        ...
        for (let i = 1; i <= 3; i += 1) {
            ...
            if (!user) {
                user = yield prisma.user.create({
                    data: { email, password: yield bcrypt_1.default.hash(PASS, 10), role: 'STUDENT', isVerified: true }
                });
            }
            let student = yield prisma.student.findUnique({ where: { userId: user.id } });
            if (!student) {
            ...
            let resume = yield prisma.resume.findFirst({ where: { studentId: student.id } });
            if (!resume) {
                resume = yield prisma.resume.create({
                    data: {
                        studentId: student.id,
        ...
        let job = yield prisma.job.findFirst({ where: { companyName: 'Round3 Systems', role: 'SDE I' } });
        if (!job) {
            job = yield prisma.job.create({
                data: {
                    role: 'SDE I',
        ...
        yield prisma.jobStage.deleteMany({ where: { jobId: job.id } });
        yield prisma.jobStage.createMany({
            data: [
        ...
        for (let i = 0; i < students.length; i += 1) {
            ...
            const existing = yield prisma.jobApplication.findFirst({
            ...
            yield prisma.profileLock.updateMany({
