...
export function getAtsLlmBaseUrl(): string | undefined {
    const explicit =
        normalizeEnvKey(process.env.ATS_LLM_BASE_URL) || normalizeEnvKey(process.env.OPENAI_API_BASE_URL);
...
export function isAtsUsingOpenRouter(): boolean {
    const base = getAtsLlmBaseUrl() || '';
    const key = getAtsLlmApiKey();
    return base.includes('openrouter.ai') || key?.startsWith('sk-or-v1-') === true;
}

/**
 * Chat model for ATS JSON analysis. Defaults to Qwen 3.6 Plus on OpenRouter; otherwise gpt-4o-mini for api.openai.com.
 */
export function getAtsChatModel(): string {
    const explicit =
        normalizeEnvKey(process.env.ATS_LLM_MODEL) || normalizeEnvKey(process.env.OPENAI_ATS_MODEL);
    if (explicit) return explicit;
 ...
 */
export function getAtsChatModelCandidates(): string[] {
