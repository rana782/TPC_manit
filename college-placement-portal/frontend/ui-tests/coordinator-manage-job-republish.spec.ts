...
test.beforeAll(() => {
  [SUCCESS_DIR, EDIT_DIR, EDGE_DIR].forEach((d) => fs.mkdirSync(d, { recursive: true }));
});

test('coordinator can edit and republish published job', async ({ page }) => {
  const seedRes = await page.request.get(`${API_BASE}/seed/seed-ui`);
  expect(seedRes.ok()).toBeTruthy();
  await enableBackendProxy(page);

  const suffix = Date.now().toString().slice(-5);
  const role = `Coord Editable ${suffix}`;
  const updated = `Coord Republished ${suffix}`;

  // Create a published job using SPOC
  ...
  await form.locator('input[type="text"]').nth(0).fill('CoordCorp');
  await form.locator('input[type="text"]').nth(1).fill(role);
  await page.getByPlaceholder('e.g. 12 LPA').fill('10 LPA');
  await form.locator('textarea').first().fill('Job to be edited by coordinator.');
  ...
  // Coordinator edits and republishes
  await login(page, COORD_EMAIL);
  await page.goto('/jobs-management', { waitUntil: 'networkidle' });
  await expect(page.locator('[data-testid="spoc-job-card"]').filter({ hasText: role }).first()).toBeVisible({ timeout: 20000 });
  await page.locator('[data-testid="spoc-job-card"]').filter({ hasText: role }).first().locator('button[title="Edit"]').click();
  await page.locator('form#jobForm input[type="text"]').nth(1).fill(updated);
